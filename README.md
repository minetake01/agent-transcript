...
`origin` は認証情報を除き、`git@host:path` や `ssh://` を `https://host/path` にし、ホストだけ小文字にして末尾の `.git` を除いた文字列です。origin の無いセッションはアップロードせず、`ingest` は最後にその一覧を出して終了コード 1 になります。
...
`ingest` は、前回の fingerprint が一致してその内容ハッシュがカタログにあるセッションを読みません。fingerprint が無い、または変わったセッションだけ本文を読み、内容ハッシュが新しいものだけを送ります。カタログが参照しなくなったオブジェクトは `gc` で削除します。
